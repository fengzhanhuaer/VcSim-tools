...
public:
	...
	bool8 SetTagValue(uint64 Val) { return ctxtag.value.u64 = Val; }
...
public:
	...
	virtual void Ctx2String(EthFrameHdr* hdr, void* ctx, void*& ctxEnd)
	{
		...
		frmDes.Printf("%02X.%02X.%02X.%02X.%02X.%02X", pu8[0], pu8[1], pu8[2], pu8[3], pu8[4], pu8[5]);
	}
	...
	virtual void SetFilter(bool Enable, FilterType Type, const char* Ctx)
	{
		...
		TransString ts;
		ts.SetBuf(Ctx);
		filterValue.value.u8bytes.u8[0] = ts.GetHex('.', 3);
		filterValue.value.u8bytes.u8[1] = ts.GetHex('.', 3);
		filterValue.value.u8bytes.u8[2] = ts.GetHex('.', 3);
		filterValue.value.u8bytes.u8[3] = ts.GetHex('.', 3);
		filterValue.value.u8bytes.u8[4] = ts.GetHex('.', 3);
		filterValue.value.u8bytes.u8[5] = ts.GetHex('.', 3);
	}
};
...
public:
	...
	virtual void Ctx2String(EthFrameHdr* hdr, void* ctx, void*& ctxEnd)
	{
		...
		frmDes.Clear();
		UpdateLocalInfo(hdr, ctx, ctxEnd);
		if ((uint32)this->ctxend >= hdr->frameLen)
		{
			frmDes.Clear();
			return;
		}
